...
def main():
    ...
    if not extras or len(extras)!=int(d['selected_count']):
        raise SystemExit('invalid discovery selected-extra payload')
    cfg={'schema':1,'max_degree':12,'primes':list(args.primes),'seed_start':args.seed_start,
         ...
         'extras':extras,'max_bytes':5_583_457_484,'max_multiply_adds':200_000_000_000,
         'exact_blas':True,'lie_depth':3,'flow_analysis':False}
    args.output.parent.mkdir(parents=True,exist_ok=True)
    args.output.write_text(json.dumps(cfg,indent=2)+'\n')
    print('PASS: STEP 3C FRESH SELECTED-SET CONFIG')
    print('selected extras:',len(extras));print('fresh primes:',args.primes);print('WROTE:',args.output)
